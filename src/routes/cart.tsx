...
export const Route = createFileRoute("/cart")({
  validateSearch: (s: Record<string, unknown>): CartSearch => ({
    success: s.success === true || s.success === "true" || s.success === 1,
  }),
...
function CartPage() {
  ...
  const [showOrderSuccess, setShowOrderSuccess] = useState(Boolean(search.success));
  ...
  return (
    <>
      <AnimatePresence>
        {showOrderSuccess && search.success && (
          <motion.div
          ...
          >
            <div className="flex items-start gap-3 rounded-2xl border border-primary/20 bg-primary text-primary-foreground px-4 py-3 shadow-elegant">
              ...
              <p className="text-sm font-medium leading-snug flex-1">{t("order_received_toast")}</p>
              <button
                type="button"
                onClick={() => {
                  setShowOrderSuccess(false);
